...
int	mutex_setup(t_setup *set, t_mutex *mutexes)
{
	...
	mutexes->mtx_fork = init_mutex_tab(num);
	if (!mutexes->mtx_fork)
		return (free(mutexes->mtx_printf), free(mutexes->mtx_death), 0);
	mutexes->mtx_eaten = init_mutex_tab(num);////
	if (!mutexes->mtx_eaten)//destroy previous mutex 
		return (free(mutexes->mtx_printf), free(mutexes->mtx_death), 0);
	mutexes->mtx_lasteat = init_mutex_tab(num);////
	if (!mutexes->mtx_lasteat)//destroy previous mutex
		return (free(mutexes->mtx_printf), free(mutexes->mtx_death), 0);
